...
public class Page {
	...
	public Page( int currentPage, int totalCount,	int pageUnit, int pageSize ) { // ( 현재페이지, 총게시물, 하단번호수, 게시물수)
		...
		this.beginUnitPage = ( (currentPage-1) / pageUnit ) * pageUnit +1 ; // 최소 한개는 보여야 하기 때문에
		
		if( maxPage <= pageUnit ){ //  만약 2<=3
			this.endUnitPage = maxPage; // 마지막페이지 = 2
		}else{ // 만약 6>3
			this.endUnitPage = beginUnitPage + (pageUnit -1); // 마지막페이지 = 5 
			if( maxPage <= endUnitPage){ // 6<=5
				this.endUnitPage = maxPage;  // 마지막페이지 = 6
			}
		}
	}
